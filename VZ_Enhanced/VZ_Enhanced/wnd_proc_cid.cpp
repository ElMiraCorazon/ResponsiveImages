...
displayinfo *edit_fcid_di = NULL;	// Display the caler ID we want to forward (when forwarding a caller ID in the call log listview).
displayinfo *edit_icid_di = NULL;	// Display the caler ID we want to ignore (when ignoring a caller ID in the call log listview).
forwardcidinfo *edit_cid_fi = NULL;	// Display the caller ID we want to edit (when editing a caller ID in the forward list listview).
...
bool multiple_fcaller_ids = false;	// Change the caller ID if we're adding multiple caller IDs.
...
LRESULT CALLBACK CIDWndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    switch ( msg )
    {
		case WM_CREATE:
		{
			...
			// 0 = ignore, 1 = show forward edit control
			...
			if ( enable_items == 1 )
			{
				...
				HWND g_hWnd_number = _CreateWindowExW( WS_EX_CLIENTEDGE, WC_EDIT, NULL, ES_AUTOHSCROLL | ES_CENTER | ES_NOHIDESEL | ES_NUMBER | WS_CHILD | WS_TABSTOP | WS_VISIBLE, 20, 75, ( rc.right - rc.left ) - 40, 20, hWnd, ( HMENU )EDIT_CID_NUMBER, NULL, NULL );
			...
			g_hWnd_chk_match_case = _CreateWindowW( WC_BUTTON, ST_Match_case, BS_AUTOCHECKBOX | WS_CHILD | WS_TABSTOP | WS_VISIBLE, 20, 60 + ( enable_items == 1 ? 40 : 0 ), rc.right - 40, 20, hWnd, ( HMENU )BTN_MATCH_CASE, NULL, NULL );
			g_hWnd_chk_match_whole_word = _CreateWindowW( WC_BUTTON, ST_Match_whole_word, BS_AUTOCHECKBOX | WS_CHILD | WS_TABSTOP | WS_VISIBLE, 20, 80 + ( enable_items == 1 ? 40 : 0 ), rc.right - 40, 20, hWnd, ( HMENU )BTN_MATCH_WHOLE_WORD, NULL, NULL );
			...
			switch( LOWORD( wParam ) )
			{
				case BTN_CID_ACTION:
				{
					...
					if ( hWnd == g_hWnd_forward_cid )
					{
						...
						fcidui->fcidi = NULL;
						fcidui->action = 0;	// Add = 0, 1 = Remove, 2 = Add all tree items, 3 = Update
						fcidui->hWnd = g_hWnd_forward_cid_list;	// Update forward cid list items.
						fcidui->caller_id = NULL;
						fcidui->forward_to = NULL;
						fcidui->match_case = match_case;
						fcidui->match_whole_word = match_whole_word;

						if ( edit_fcid_di != NULL )	// Add item from call log listview to forward_cid_list and forward cid list listview.
						{
							fcidui->hWnd = g_hWnd_call_log;	// Update call log items.
						}
						else if ( edit_cid_fi != NULL )	// Update all the forward information.
						{
							fcidui->fcidi = edit_cid_fi;

							fcidui->action = 3;	// Update
						}
						else	// Add item to forward cid list listview and update items in call log listview.
						{
							char *forward_cid = ( char * )GlobalAlloc( GMEM_FIXED, sizeof( char ) * ( length + 1 ) );
							_memcpy_s( forward_cid, length + 1, caller_id, length );
							forward_cid[ length ] = 0;	// Sanity

							fcidui->caller_id = forward_cid;
						}

						char *forward_to = ( char * )GlobalAlloc( GMEM_FIXED, sizeof( char ) * ( length2 + 1 ) );
						_memcpy_s( forward_to, length2 + 1, number, length2 );
						forward_to[ length2 ] = 0;	// Sanity

						fcidui->forward_to = forward_to;
					...
					else if ( hWnd == g_hWnd_ignore_cid )
					{
		...
		case WM_PROPAGATE:
		{
			if ( wParam == 1 || wParam == 5 )	// Forward caller ID.
			{
				edit_cid_fi = NULL;

				edit_fcid_di = ( displayinfo * )lParam;
				multiple_fcaller_ids = ( wParam == 5 ? true : false );

				_SendMessageW( _GetDlgItem( hWnd, BTN_CID_ACTION ), WM_SETTEXT, 0, ( LPARAM )ST_Forward_Caller_ID_Name );
				...
				else	// If edit_fcid_di is NULL, then we're adding new caller IDs from the forward cid list listview.
				{
			...
			else if ( wParam == 2 || wParam == 6 )	// Ignore caller ID.
			{
				edit_cid_ii = NULL;

				edit_icid_di = ( displayinfo * )lParam;
				multiple_icaller_ids = ( wParam == 6 ? true : false );
			...
			{
				multiple_fcaller_ids = false;
				edit_fcid_di = NULL;

				edit_cid_fi = ( forwardcidinfo * )lParam;

				if ( edit_cid_fi != NULL )	// If not NULL, then allow update.
				{
			...
			else if ( wParam == 4 )	// Update ignore information.
			{
